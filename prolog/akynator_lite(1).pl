...
android(c_sharp, 0).
android(python, 0).
android(c_plu_plus, 0).
android(f_sharp, 0).
android(prolog, 0).
android(c, 0).
android(asm, 0).
android(javascript, 0).
android(kotlin, 1).
android(swift, 0).
...
question8(X8):- write("Is your language primarily used for Android development?"), nl,
                write("1. Yes"), nl,
                write("0. NO"), nl,
                read(X8).
...
pr:-	question1(X1),question2(X2),question3(X3),question4(X4),
		...
		cross(X,X5),visual(X,X6),mobile(X,X7),android(X,X8),
		write(X).
